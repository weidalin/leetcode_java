package written_examination.intern.Ctrip.Ctrip2_winner;
...
public class Main_2 {
    ...
    ******************************开始写代码******************************/
    ...
    static int winner(int[] s, int[] t) {
        ...
        dfs(i, j, list_s, list_t);

        return real_i+1;
    }

    private static void dfs(int i, int j, LinkedList<Integer> list_s, LinkedList<Integer> list_t) {
        if(!list_s.containsAll(list_t) || i > j || list_s.size() <= 0){
            return;

        }else{
//            System.out.println("i : j < real_i : real_j: " + i + " : " + j + " < " + real_i +" : " + real_j);
            if(j - i < real_j - real_i){
                real_j = j;
                real_i = i;
            }else if(j - i == real_j - real_i && real_i > i){
                real_j = j;
                real_i = i;
            }
            LinkedList tmp = new LinkedList(list_s);
            list_s.removeLast();
            dfs(i, j - 1,list_s, list_t);
            tmp.removeFirst();
            dfs(i + 1, j, tmp, list_t);
        }
